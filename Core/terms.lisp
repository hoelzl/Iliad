...
#+5am
(5am:in-suite poem-syntax-suite)
...
(defclass variable-term (term name-mixin source-mixin)
  ((unique-name
   ...
   (globalp
    :accessor globalp :initarg :globalp :initform nil
...
(defun make-variable-term (name sort context
                           &key (intern t) (is-bound-p nil) (globalp nil))
  (when (stringp name)
...
(defun make-anonymous-variable-term (sort context &key (is-bound-p nil) (globalp nil))
  (let* ((name (make-symbol (format nil "?_V~A.~A"
                                    (incf *unique-variable-counter*)
                                    sort)))
         (result (make-variable-term
                  name sort context
                  :is-bound-p is-bound-p :globalp globalp)))
...
(defclass ternary-term (known-application-term)
  ((term-arg1 :accessor term-arg1 :initarg :term-arg1)
   (term-arg2 :accessor term-arg2 :initarg :term-arg2)
   (term-arg3 :accessor term-arg3 :initarg :term-arg3))
  (:documentation
...
(defmethod shared-initialize :after ((self ternary-term) slot-names
                                     &key arguments
                                          (term-arg1 nil term-arg1-supplied-p)
                                          (term-arg2 nil term-arg2-supplied-p)
                                          (term-arg3 nil term-arg3-supplied-p))
  (declare (ignore slot-names term-arg1 term-arg2 term-arg3))
  "Provide :ARGUMENTS as additional init-keyword.  Its argument must be a list
of length 3, and it must not be provided when either of the :TERM-ARG1, :TERM-ARG2
or :TERM-ARG3 init-keywords is also provided."
  (when arguments
    (assert (and (consp arguments) (= (length arguments) 3))
            (arguments)
            "Keyword argument :ARGUMENTS is not a list of length 3: ~A." arguments)
    (assert (and (not term-arg1-supplied-p) (not term-arg2-supplied-p) (not term-arg3-supplied-p))
            ()
            "Cannot provide both :TERM-ARG1/:TERM-ARG2/:TERM-ARG3 and :ARGUMENTS keyword arguments.")
    (setf (term-arg1 self) (first arguments)
          (term-arg2 self) (second arguments)
          (term-arg3 self) (third arguments))))

(defmethod arguments ((term ternary-term))
  (list (term-arg1 term) (term-arg2 term) (term-arg3 term)))
...
(defmethod (setf arguments) (new-value (term ternary-term))
  ...
  (setf (term-arg1 term) (first new-value)
        (term-arg2 term) (second new-value)
        (term-arg3 term) (third new-value)))
